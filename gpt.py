...
parser.add_argument(
    "--corpus",
    default="data/crime_and_punishment.txt",
    help="Plain UTF-8 text file as training data. Gets split 90/10",
)
...
parser.add_argument(
    "--n_estimate_steps",
    default=100,
    help="The number of steps to take in loss estimation (it is probabilistic)",
)
parser.add_argument(
    "--n_gen_tokens",
    type=int,
    default=500,
    help="The number of tokens to generate during inference.",
)
...
def create_batch(
    batch_size: int, context_length: int, split: str
) -> Tuple[Integer[Tensor, "B T"], Integer[Tensor, "B T"]]:
...
@jaxtyped(typechecker=typechecker)
def estimate_loss(
    model: Module, eval_iters: int, batch_size: int, context_length: int
) -> Dict[str, float]:
...
class Attention(Module):
    ...
    def __init__(
        self, input_dim: int, output_dim: int, dropout: float, context_length: int
    ) -> None:
        super().__init__()
...
class MultiHeadAttention(Module):
    ...
    def __init__(
        ...
        self.heads = ModuleList(
            Attention(input_dim, int(output_dim / num_heads), dropout, context_length)
...
class TransformerLayer(Module):
    ...
    def __init__(
        ...
        self.mh_attention = MultiHeadAttention(
            input_dim, output_dim, num_heads, dropout, context_length
        )
...
class GPT(Module):
    def __init__(
        ...
        self.layers = ModuleList(
            [
                TransformerLayer(hdim, hdim, num_heads, dropout, context_length)
    ...
    def generate(
        ...
        for _ in range(max_output_length - T):
            ...
            if payg:
                ...
                if B > 1:
                    raise ValueError(
                        "Can't print-as-you-go with batch size > 1. Either set batch size to 1 or turn off payg."
                    )
    ...
    for step in tqdm.tqdm(range(args.train_steps)):
        ...
        if step % args.save_eval_every_n_steps == 0:
            ...
            result = estimate_loss(
                gpt, args.n_estimate_steps, args.batch_size, args.context_length
            )
            print(result)
